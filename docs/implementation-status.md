...
runtime identity and a complete normalized turn stream. Two frozen P10 live runs independently
failed the same three hard checks because the SDK stream contained no shell executions. The gate
...
FR-03 update (2026-09-21, D0.6): the raw runner now captures each SDK version's final wire shape
through a no-runtime fake transport, then bypasses the high-level SDK thread/turn wrapper and talks
directly to the same bundled app-server. Both 0.154.0 matrix `80715924...947d7` and temporary
0.155.1 matrix `af78f1e7...0b438` passed offline replay verification. All eight turns completed with
zero structured command invocations, yielding `RAW_THREAD_OBSERVABILITY_GAP / eligible_for_p10=false`.
This makes the high-level wrapper unnecessary to reproduce the observation, without proving that
...
variant explicitly enables `executed_tool_call_metadata`; exact 0.154.0 source proves that this
metadata is stripped for non-OpenAI provider names, so the loopback provider uses the upstream
`OpenAI` name while remaining bound to `127.0.0.1`, unauthenticated and credential-rejecting. The
second request's allowlisted projection mechanically proves exact marker output, exit 0, a nonempty
chunk id, exact nested `exec_command` arguments and binding to the outer `exec` call. One matched
...
The current P14a implementation slice and its remaining qualification gates are recorded in
...
上的冻结 structured synthetic Evidence proposal 经 typed service facades、compiler、有界 job、既有 native-Qlib、
...
[`p11-progress.md`](p11-progress.md)。该证据不包含 production MCP transport、真实 Agent 生成的
proposal 或 Agent-to-Data-qualified 组合运行；三者不得从 P10 mock transport 与 P11 direct-facade
fixture 的分别通过中推导出来。
...
P12 implementation      60b8c811eba175af9914d55668f341a7b332d6f7
P12 Evidence staging    e63ecc4b73c166eb202676d9776ff9d24aeb04eab8aaf9216810585bddf6d301
P12 Evidence Store      c55e9ba41c60c8443c979396277cfad5ed5d4f7c2344a4348a91b1b63b566a7f
P12 freeze report       9a97dc04ba74de26bbd3cd02932db9f90c911fac001cc773651d68643db374f4
```
